    ...
    cart.forEach((item) => {
      ...
      cartRowContents += `<div class="container">
        ...
        parseFloat(item.display_price).toLocaleString()
                    ...
                    <div class="col-md-2 mt-4">
                        <button class="btn btn-xs btn-danger remove-cart-item"  data-cart_id=${item.cart_id
      ...
      if (add_ons_items.length > 0) {
        ...
        for (var j = 0; j < add_ons_items.length; j++) {
          cartRowContents +=
            `<div class="font-size-sm text-body cart-addons-name">
                                    <span>` +
            add_ons_items[j] +
            `</span>
...
function update_cart_total() {
  ...
  const test_cart_total = parseFloat(total.cart_total);
  const test_add_ons_total = parseFloat(add_ons);
  const add_ons_total = test_cart_total + test_add_ons_total;
...
function get_products(
  ...
  $.ajax({
    type: "GET",
    url: `${base_url}admin/point_of_sale/get_products?category_id=${category_id}&limit=${limit}&offset=${offset}&search=${search_parameter}`,
    dataType: "json",
    beforeSend: function () {
      $("#get_products").html(
...
$(document).on("click", ".modal-trigger", function () {
  ...
  $.ajax({
    type: "GET",
    url: `${base_url}admin/point_of_sale/add_ons_list?product_id=${id}`,
    dataType: "json",
...
function add_items(e) {
  ...
  var partner_id = $("#session_branch_id").val();
...
function add_to_cart(cart_item) {
  ...
  var cart_id = "cart_" + new Date().getTime() + Math.floor(Math.random() * 1000); // Unique ID
...
function display_products(products = "") {
  ...
  if (products !== null && products.length > 0) {
    for (i = 0; i < products.length; i++) {
      ...
      let dispayclass = variants.length == 1 ? "d-none" : ";"
      ...
      display_products +=
        ...
        " <a href=" +
        base_url +
        "admin/product/view_product?edit_id=" +
        products[i].id +
        ' target="_BLANK"><b class="text text-dark pos-product-name">' +
        stripslashes(products[i].name) +
        "</b></a>" +
        " </span>" +
      ...
      let input = '<select class="form-control mt-4 product-variants variant_value ' + dispayclass + '" id="change"  >';
      ...
      if (variants.length == 1) {
        let singledisplayvalue = variant_values +
          currency +
          " " +
          parseFloat(variant_price).toLocaleString()
        input += '<input class="form-control mt-4 " value="' + singledisplayvalue + '" disabled>'
      }


      display_products = display_products + input + "</div>" +
...
var session_user_id = $("#session_branch_id").val();
var cart = localStorage.getItem("cart");
...
$(".select_user").select2({
  ajax: {
    url: base_url + "admin/point_of_sale/get_users",
    type: "GET",
    dataType: "json",
    delay: 250,
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.error == false) {
        ...
        // Split the error messages by <div> tags
        var errorMessages = result.message.match(/<div>(.*?)<\/div>/g);

        // Loop through each extracted <div> and display the messages separately
        if (errorMessages) {
          errorMessages.forEach(function (msg) {
            // Remove the <div> and </div> tags
            var cleanMessage = msg.replace(/<\/?div>/g, '');

            // Show the message in iziToast error notification
            iziToast.error({
              message: cleanMessage
            });
          });
        } else {
          // If there's no HTML, display the raw message
          iziToast.error({
            message: result.message
          });
        }
