...
/*
   ...
   33. fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $rider_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false,$download_thermal_invoice = false)
...
function verify_app_request()
{
    ...
    $user_token = " ";
    try {
        $user_id = $t->jwt->decode_unsafe($token)->user_id;
...
function generate_tokens($identity, $email = null)
{
    ...
    if (!empty($identity)) {
        $user_id = fetch_details(['mobile' => $identity], "users", "id")[0]['id'];
    } else {
        $user_id = fetch_details(['email' => $email], "users", "id")[0]['id'];
    }
    $payload = [
...
function verify_tokens()
{
    ...
    if (!empty($token)) {
        ...
        try {
            $user_id = $t->jwt->decode_unsafe($token)->user_id;
            $user_token = fetch_details(['id' => $user_id], 'users')[0]['apikey'];
...
function fetch_details($where = NULL, $table = "", $fields = '*', $limit = '', $offset = '', $sort = '', $order = '', $where_in_key = '', $where_in_value = '', $join_table_with_alias = "", $on = "", $is_left = false, $group_by = NULL, $or_where = NULL)
{
    $t = &get_instance();
    $t->db->select($fields);
...
function fetch_product($branch_id = NULL, $user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $sort_by = NULL, $cart_id = NULL, $product_variant_id = NULL)
{
    ...
    if (isset($filter) && !empty($filter['search'])) {
        $categoryName = trim($filter['search']);
        $category = $t->db->select('id')
            ->from('categories')
    ...
    if (isset($filter['branch_id']) && !empty($filter['branch_id']) && $filter['branch_id'] != "") {

        $where['p.branch_id'] = $filter['branch_id'];
    } else {
    ...
    // vegetarian filter
    if (isset($filter) && isset($filter['vegetarian']) && $filter['vegetarian'] != "") {
        if ($filter['vegetarian'] == "3") {
            $t->db->where('b.type', $filter['vegetarian']);
        } else {
    ...
    if (isset($product_variant_id) && !empty($product_variant_id)) {
        $t->db->where('pv.id', $product_variant_id);
    }

    if (isset($filter) && !empty($filter['category_slug']) && $filter['category_slug'] != "") {
    ...
    if (isset($filter['discount']) && !empty($filter['discount']) && $filter['discount'] != "") {
        ...
        if (isset($filter['product_variant_ids'])) {
            $product_varient = implode(",", $filter['product_variant_ids']);
            $t->db->order_by('FIELD(pv.id,' . $product_varient . ')', '', false);
    ...
    if (isset($filter) && isset($filter['vegetarian']) && $filter['vegetarian'] != "") {
        if ($filter['vegetarian'] == "3") {
            ...
            $t->db->where('p.indicator ', $filter['vegetarian']);
            $t->db->where_in('p.indicator ', [$filter['vegetarian'], 3]);
        }
    }
    ...
    if (isset($filter) && !empty($filter['product_variant_ids'])) {
        if (is_array($filter['product_variant_ids'])) {
            $product_count->where_in('pv.id', $filter['product_variant_ids']);
        }
    }

    if (isset($product_variant_id) && !empty($product_variant_id)) {
        $product_count->where('pv.id', $product_variant_id);
    ...
    $get_bestselling_products = best_seller();
    if (!empty($product)) {
        $t->load->model('rating_model');
        for ($i = 0; $i < count($product); $i++) {
            $is_best_seller = "0"; // Assume not a best seller
            foreach ($get_bestselling_products as $bestselling_product) {
                if ($bestselling_product['product_id'] == $product[$i]['id']) {
                    $is_best_seller = "1"; // Mark as best seller
                    break; // No need to check further
                }
            }
            $product[$i]['best_seller'] = $is_best_seller;
           
            $product[$i]['is_branch_open'] = isset($filter['branch_id']) ? ((is_restro_open($filter['branch_id']) == true) ? "1" : "0") : "0";
            ...
            $relative_path = isset($product[$i]['image']) && !empty($product[$i]['image']) ? $product[$i]['image'] : [];
            $product[$i]['relative_path'] = $relative_path;
            ...
            $settings = get_settings('system_settings', true);
            $product[$i]['tax_id'] = isset($settings['tax']) && !empty($settings['tax']) ? $settings['tax'] : '';
            $tax_name = fetch_details(['id' => $settings['tax']], 'taxes', 'title');
            $product[$i]['tax_name'] = isset($tax_name[0]['title']) ? $tax_name[0]['title'] : "";
            ...
            $product[$i]['category_slug'] = isset($product[$i]['category_slug']) && !empty($product[$i]['category_slug']) ? output_escaping($product[$i]['category_slug']) : ''; 
            ...
            if (isset($branch_id) && !empty($branch_id)) {
                $product[$i]['branch_detail'] = fetch_details(['id' => $branch_id], 'branch', '*');
            }
            ...
            if (!empty($product[$i]['variants'])) {
                ...
                for ($k = 0; $k < count($product[$i]['variants']); $k++) {
                    ...
                    $product[$i]['variants'][$k]['price'] = (isset($product[$i]['variants'][$k]['price']) && !empty($product[$i]['variants'][$k]['price'])) ? strval($product[$i]['variants'][$k]['price']) : "0";
                    ...
                    if (isset($user_id) && $user_id != NULL) {
                        /* get cart total */
                        $current_branch_id = !empty($filter['branch_id']) && isset($filter['branch_id']) ? $filter['branch_id'] : $_POST['branch_id'];
                       

                        $user_cart_data = get_cart_total($user_id, $product[$i]['variants'][$k]['id'], '', '', $current_branch_id, $cart_id);
            ...
            $is_purchased_count = isset($is_purchased_count) ? array_count_values($is_purchased_count) : [];
            $is_purchased_count = isset($is_purchased_count) ? array_keys(array($is_purchased_count)) : "";
            ...
            if (($product[$i]['stock_type'] != null && !empty($product[$i]['stock_type']))) {
                ...
                if ($product[$i]['stock_type'] == 1 || $product[$i]['stock_type'] == 2) {
                    $counts = isset($count_stock) ? array_count_values($count_stock) : [];
...
{
    $t = &get_instance();
    $t->db->select('p.id as product_id, p.name, COUNT(oi.product_variant_id) as total_sold');
    $t->db->from('order_items oi');
    $t->db->join('product_variants pv', 'oi.product_variant_id = pv.id');
    $t->db->join('products p', 'pv.product_id = p.id');
    $t->db->group_by('p.id, p.name');
    $t->db->order_by('total_sold', 'DESC');
    $t->db->limit(10);
    $query = $t->db->get();
...
//validate_promo_code
function validate_promo_code($promo_code, $user_id, $final_total, $branch_id)
{

    if (isset($promo_code) && !empty($promo_code) && isset($branch_id) && !empty($branch_id)) {
        $t = &get_instance();
        ...
        $promo_code = $t->db->select('pc.*,count(o.id) as promo_used_counter ,( SELECT count(user_id) from orders where user_id =' . $user_id . ' and promo_code ="' . $promo_code . '") as user_promo_usage_counter ')
            ...
            ->where(['pc.promo_code' => $promo_code, 'pc.status' => '1', ' start_date <= ' => date('Y-m-d'), '  end_date >= ' => date('Y-m-d'), 'pc.branch_id' => $branch_id])
...
function validate_order_status($order_ids, $status, $table = 'order_items', $user_id = null)
{
    ...
    $cancelable_till = '';
    $check_status = ['pending', 'confirmed', 'preparing', 'out_for_delivery', 'delivered', 'cancelled', 'ready_for_pickup'];
    $group = array('admin', 'rider', 'partner');
    if (in_array(strtolower(trim($status)), $check_status)) {

        $t->db->select('p.*,o.active_status,pv.*,oi.id as order_item_id,o.user_id as user_id,oi.product_variant_id as product_variant_id, o.status as order_status,oi.order_id as order_id')
        ...
        for ($i = 0; $i < count($product_data); $i++) {
            ...
            $product_data[$i]['order_status'] = isset($product_data[$i]['order_status']) ? json_decode($product_data[$i]['order_status'], true):"";
            $order_status = isset($product_data[$i]['order_status']) ? array_column($product_data[$i]['order_status'], '0') : [];
        ...
        if ($is_posted_status_set == true) {
            ...
            $response['error'] = true;
            $response['message'] = "Order is already marked as $status_name. You cannot set it again!";
            $response['data'] = array();
            return $response;
        }
        ...
        /** check if user exist */
        if (isset($product_data) && !empty($product_data) && $status != "cancelled") {
            if (!is_exist(['id' => $product_data[0]['user_id']], "users")) {
                $response['error'] = true;
                $response['message'] = "User was deleted! So, order can not be proccessed further.";
                $response['data'] = array();
                return $response;
            }
        }
...
function get_cart_total($user_id, $product_variant_id = false, $is_saved_for_later = '0', $address_id = '', $branch_id = NULL)
{
    
    $t = &get_instance();
    $t->db->select('c.id as cart_id, (select sum(c.qty)  from cart c join product_variants pv on c.product_variant_id=pv.id join products p on p.id=pv.product_id where c.user_id="' . $user_id . '" and qty!=0  and  is_saved_for_later = "' . $is_saved_for_later . '" and p.status=1 AND pv.status=1) as total_items,(select count(c.id) from cart c join product_variants pv on c.product_variant_id=pv.id join products p on p.id=pv.product_id where c.user_id="' . $user_id . '" and qty!=0 and  is_saved_for_later = "' . $is_saved_for_later . '" and p.status=1 AND pv.status=1) as cart_count,`c`.qty,p.is_prices_inclusive_tax,p.cod_allowed,p.is_spicy,p.minimum_order_quantity,p.slug,p.total_allowed_quantity, p.name, p.image,p.short_description,`c`.user_id,pv.*,tax.percentage as tax_percentage,tax.title as tax_title, pv.availability');
    $t->db->where('(pv.availability IS NULL OR pv.availability = 1)');
    ...
    $t->db->where(['p.status' => '1', 'pv.status' => 1, 'c.branch_id' => $branch_id]);
    ...
    for ($i = 0; $i < count($data); $i++) {
        ...
        if (floatval($data[$i]['special_price']) > 0) {
            $total[$i] = floatval($data[$i]['special_price']) * $data[$i]['qty'];
        } else {
        ...
        $data[$i]['special_price'] = $data[$i]['special_price'];
    ...
    $amount_with_tax = $total * ($tax / 100);
    $overall_amount = $total + $amount_with_tax;
   
    $data['sub_total'] = strval($total);
    $data['quantity'] = strval(array_sum($quantity));
    $data['tax_percentage'] = strval($tax);
    $data['tax_amount'] = strval($amount_with_tax);
    $data['total_arr'] = $amount_with_tax;
    $data['variant_id'] = $variant_id;
    $data['overall_amount'] = strval($overall_amount);
...
function get_product_add_ons($variant_id, $product_id, $user_id, $cart_id)
{
    ...
    $data = fetch_details(
        [
            ...
            "ca.product_variant_id" => $variant_id,
            "ca.cart_id" => $cart_id
        ...
        for ($i = 0; $i < count($image_type); $i++) {
            ...
            if (file_exists($source_path . $image_name)) {  //check if the image file exist
                foreach ($image_size as $image_size_key => $image_size_value) {
                    ...
                    $n_w = $image_size_value['width']; // destination image's width 
                    $n_h = $image_size_value['height']; // destination image's height
...
function has_permissions($role, $module)
{
    ...
    if (!empty($userData)) {
        ...
        if (intval($userData[0]['role']) > 0) {
            $permissions = isset($userData[0]['permissions']) ? json_decode($userData[0]['permissions'], 1) : [];
...
function fetch_orders($order_id = NULL, $user_id = NULL, $status = NULL, $rider_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $download_thermal_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $branch_id = null, $is_pending = false, $rider_city_id = NULL, $rejected_rider = NULL)
{
    ...
    $sort = isset($sort) && !empty($sort) && is_string($sort) ? trim($sort) : 'o.id';
    $order = isset($order) && in_array(strtolower($order), ['asc', 'desc']) ? strtolower(trim($order)) : 'asc';

    // Apply order_by with valid parameters
    ...
    if(isset($order_details[0]['branch_id'])){
        ...
        $branch_details = fetch_details(['id' => $order_details[0]['branch_id']], 'branch', '*');
    }
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        from order_items where order_id = oi.order_id ) as order_counter')
            ...
            ->join('products p', 'pv.product_id=p.id', 'left')
            ->join('branch b', 'b.id=oi.branch_id');

        $t->db->or_where_in('oi.order_id', $order_details[$i]['id']);
        if (isset($branch_id) && $branch_id != null) {
            $t->db->where('oi.branch_id=' . $branch_id);
        }
        $order_item_data = $t->db->get('order_items oi')->result_array();

        $order_details[$i]['status'] = isset($order_details[$i]['status']) ? json_decode($order_details[$i]['status']) : [];
        for ($k = 0; $k < count(array($order_details[$i]['status'])); $k++) {
        ...
        if (isset($order_details[$i]['cancel_by']) && !empty($order_details[$i]['cancel_by']) && isset($order_details[$i]['reason']) && !empty($order_details[$i]['reason'])) {
            $username = fetch_details(['u.id' => $order_details[$i]['cancel_by']], "users u", "username");
            $order_details[$i]['cancel_by'] = $username[0]['username'] . "( " . $t->ion_auth->get_users_groups($order_details[$i]['cancel_by'])->row()->description . " )";
            $order_details[$i]['reason'] = (isset($order_details[$i]['reason']) && !empty($order_details[$i]['reason'])) ? $order_details[$i]['reason'] : "";
        } else {
            $order_details[$i]['cancel_by'] = "";
            $order_details[$i]['reason'] = "";
        }
        if (isset($order_details[$i]['rider_id']) && !empty($order_details[$i]['rider_id'])) {
            $rider_data = fetch_details(['id' => $order_details[$i]['rider_id']], "users", "mobile,username,image,rating,no_of_ratings,balance,rider_cancel_order");
            $oredr_wise_rider_rating = fetch_details(['order_id' => $order_details[$i]['id']], 'rider_rating', 'rating');
            $order_details[$i]['order_rider_rating'] = !empty($oredr_wise_rider_rating) ? $oredr_wise_rider_rating[0]['rating'] : "0.0";
            $order_details[$i]['rider_mobile'] = $rider_data[0]['mobile'];
            $order_details[$i]['rider_name'] = $rider_data[0]['username'];
            $order_details[$i]['rider_cancel_order'] = $rider_data[0]['rider_cancel_order'];
            $order_details[$i]['rider_image'] = (isset($rider_data[0]['image']) && !empty($rider_data[0]['image'])) ? base_url() . $rider_data[0]['image'] : base_url() . NO_PROFILE_IMAGE;
            ...
            $order_details[$i]['rider_image'] = base_url() . NO_PROFILE_IMAGE;
        ...
        $order_details[$i]['order_product_rating'] = get_average_rating_by_order($order_details[$i]['id']);
        $total_tax_percent = $total_tax_amount = $item_subtotal = 0.0;
        for ($k = 0; $k < count($order_item_data); $k++) {
            if (!empty($order_item_data)) {
                $item_subtotal += $order_item_data[$k]['sub_total'];
                $filter['id'] = $order_item_data[$k]['branch_id'];
                ...
                if (isset($order_item_data[$k]['active_status']) && !in_array($order_item_data[$k]['active_status'], ['returned', 'cancelled'])) {
                    $total_tax_percent = $order_item_data[$k]['tax_percent'];
                    $sub_total = floatval($order_item_data[$k]['price']) * $order_item_data[$k]['quantity'];

                    $tax_amount = round($order_details[$i]['total'] * $order_item_data[$k]['tax_percent'] / 100, 2);

                    $total_tax_amount = floatval($tax_amount);
                ...
                $order_item_data[$k]['indicator'] = (!empty($order_item_data[$k]['indicator'])) ? $order_item_data[$k]['indicator'] : '0';
        ...
        if ((isset($rider_id) && $rider_id != null)) {
            ...
            // print_r($order_details);
            $order_details[$i]['final_total'] = strval($item_subtotal + $total_tax_amount  + $order_details[$i]['delivery_charge'] - $order_details[$i]['promo_discount'] + $order_details[$i]['delivery_tip']);

            $order_details[$i]['total_payable'] = strval($item_subtotal + $total_tax_amount  + $order_details[$i]['delivery_charge'] - $order_details[$i]['promo_discount'] + $order_details[$i]['delivery_tip']);
        } else {
        ...
        if (isset($order_details[$i]['profile']) && !empty($order_details[$i]['profile'])) {

            $order_details[$i]['profile'] = base_url() . USER_IMG_PATH . $order_details[$i]['profile'];
        } else {
            $order_details[$i]['profile'] = base_url() . NO_PROFILE_IMAGE;
        }
        ...
        if (!empty($branch_details)) {
            ...
            $order_details[$i]['branch_details'] = $branch_details[0];
            $order_details[$i]['branch_details']['image'] = base_url() . $branch_details[0]['image'];
            $order_details[$i]['branch_details']['is_branch_open'] = isset($branch_details[0]['id']) ? ((is_restro_open($branch_details[0]['id']) == true) ? "1" : "0") : "0";
        }
...
function get_average_rating_by_order($order_id)
{
    $t =& get_instance();
    $t->load->database();
    $t->db->select_avg('rating', 'average_rating');
    $t->db->where('order_id', $order_id);
    $query = $t->db->get('product_rating');
...
function fetch_users($id)
{
    ...
    // Check if $user_details is not empty
    if (!empty($user_details)) {
        $user_details = array_map(function ($value) {
            return $value === NULL ? "" : $value;
        }, $user_details[0]);
    } else {
        // Handle the case where no user details were found for the given user_id.
        // You can return an empty array or some other appropriate response.
        $user_details = array();
    }
...
function get_thermal_invoice_html($order_id)
{
    $t = &get_instance();
    $invoice_generated_html = '';
    $t->data['main_page'] = VIEW . 'api-thermal-invoice';
    $settings = get_settings('system_settings', true);
    $t->data['title'] = 'Invoice Management |' . $settings['app_name'];
    $t->data['meta_description'] = 'eRestro | Invoice Management';
    $is_add_ons = [];
...
function get_cities($city_id = NULL, $fields = "*", $limit = NULL, $offset = NULL)
{
    $CI = &get_instance();
    $CI->db->select($fields);
    if (!empty($city_id)) {
        $CI->db->where("FIND_IN_SET(" . $city_id . ",bordering_city_ids) !=", 0);
    }
    if (!empty($limit) || !empty($offset)) {
        $CI->db->limit($limit, $offset);
    }
    $data = $CI->db->get('cities')->result_array();
    return $data;
}
function get_favorites($user_id, $branch_id, $type = 'products', $limit = NULL, $offset = NULL)
{
    $CI = &get_instance();
    ...
    if ($type == 'products') {
        ...
        $q = $CI->db->join('products p', 'p.id=f.type_id')
            ...
            ->where('f.type', $type)
            ->select("(select count(id)  from favorites where user_id= $user_id and type='products' and branch_id= $branch_id) as total, ,f.*")
            ->group_by('f.type_id')
            ->limit($limit, $offset)
            ->get('favorites f');
        $res = $q->result_array();
        ...
        $total = $CI->db->select("count(`f`.`id`) as total")
            ->where($where)
            ->get('favorites f');
        ...
        if (!empty($res)) {
            ...
            $data['total'] = strval($total[0]['total']);
            $filter['branch_id'] = $branch_id;
            $pro_details = fetch_product("", $user_id, $filter, $product_ids, "", "", "", "", "", "", "", "", "p.id");
        ...
        $total = $CI->db->select("count(`f`.`id`) as total")
            ->join("partner_data p", "p.user_id = f.type_id")
            ->where($where)
            ->get('favorites f');
        $total = $total->result_array();

        $q = $CI->db->select("f.*,u.username as owner_name ,p.user_id, p.cooking_time,p.slug, p.partner_name, p.type, p.store_url, p.description, p.profile, p.address,p.rating as partner_rating,p.no_of_ratings,p.cooking_time as partner_cook_time")
            ->join("partner_data p", "p.user_id = f.type_id", 'LEFT')
        ...
        if (isset($favorites) && !empty($favorites)) {
            if (isset($favorites) && !empty($favorites)) {
                for ($i = 0; $i < count($favorites); $i++) {
                    $fav = $CI->db->where(['f.type_id' => $favorites[$i]['type_id'], 'f.type' => 'partners', 'f.user_id' => $user_id])->get('favorites f')->num_rows();
                    $favorites[$i]['is_favorite'] = strval($fav);
                    $favorites[$i]['profile'] = base_url($favorites[$i]['profile']);
                    $favorites[$i]['partner_profile'] = $favorites[$i]['profile'];
                    $favorites[$i]['partner_id'] = $favorites[$i]['user_id'];
                    $favorites[$i]['partner_cook_time'] = $favorites[$i]['cooking_time'] . " min";
                    $favorites[$i]['is_restro_open'] = (is_restro_open($favorites[$i]['type_id']) == true) ? "1" : "0";
                }
...
function process_refund($id, $status, $type = 'orders')
{
    ...
    $wallet_balance_cod = $order_details[0]['wallet_balance'];
    $currency = (isset($system_settings['currency']) && !empty($system_settings['currency'])) ? $system_settings['currency'] : '';
    $user_id = $order_details[0]['user_id'];
    $fcmMsg = array(
        'title' => "Amount Credited To Wallet",
    );
    $user_res = fetch_details(['id' => $user_id], 'users', 'fcm_id,web_fcm_id');
    $fcm_ids = array();
    ...
    if ($payment_method != 'cod') {
        ...
        if ($is_delivery_charge_returnable == 1) {
            ...
            $returnable_amount = $order_details[0]['total'] + $order_details[0]['delivery_charge'] + $order_details[0]['delivery_tip'] + $order_details[0]['tax_amount'] - $promo_discount;
        } else {

            $returnable_amount = $order_details[0]['total'] + $order_details[0]['tax_amount'] + $order_details[0]['delivery_tip'];
        }
        $fcmMsg = array(
            'title' => "Amount Credited To Wallet",
        ...
        if ($wallet_balance != 0) {
            if ($payment_method == 'cod') {
                /* update user's wallet */
                $returnable_amount = $wallet_balance_cod;
...
// function get_delivery_charge($address_id, $user_id, $branch_id)
// {
//     $t = &get_instance();
//     $charge = "0";
...
//         $t->db->select('b.latitude,b.longitude,p.branch_id')->join('product_variants pv', 'pv.id=c.product_variant_id')->join('products p', "pv.product_id=p.id")->join("branch b", "p.branch_id=b.id")->join("users uc", "c.user_id=uc.id");
//         $t->db->where('c.user_id', $user_id);
//         $t->db->where('b.id', $branch_id);
//         $points = $t->db->from("cart c")->get()->result_array();


//         $system_settings = get_settings('system_settings', true);
//         $distance_matrix = $system_settings['distance_matrix_api'];

//         if ($distance_matrix == 0) {
//             $query = $t->db->query("SELECT ST_Distance_Sphere(POINT(?, ?),POINT(?, ?)) AS distance", array($city_id[0]['latitude'], $city_id[0]['longitude'], $points[0]['latitude'], $points[0]['longitude']));
//             $result = $query->row();
//             $distance = round(($result->distance / 1000), 1);
//             $distance_text = round(($result->distance / 1000), 1);

//             if ($charge_method == "fixed_charge") {
//                 $charge = $get_methods[0]['fixed_charge'];
//             }
//             if ($charge_method == "per_km_charge") {
//                 $charge = (intval($get_methods[0]['per_km_charge']) * ceil($distance));
//             }
...
//             $response['distance'] = $distance_text . "km";
//             $response['duration'] = "";
...
function get_delivery_charge($address_id, $user_id, $branch_id)
{
    ...
    if (is_exist(['id' => $address_id], 'addresses')) {
        ...
        if ($distance_matrix == 0) {
            ...
            /* find distnce with google API */
            $results = find_google_map_distance($city_id[0]['latitude'], $city_id[0]['longitude'], $points[0]['latitude'], $points[0]['longitude']);
            $result = isset($results) || empty($results) ? json_decode($results,true) : [];
            // print_r($result);
            
            if (isset($result[0]['error']) && $result[0]['error']['code'] = "400") {
                // print_r("here");
                $response['error'] = true;
                $response['message'] = 'Something went wrong !';
                $response['charge'] = "0";
            ...
            if (isset($result) && !empty($result)) {
                if (isset($result[0]['error']) && $result[0]['error'] == "400") {
                    /* The request is missing an API key */
                    $response['error'] = true;
                    $response['message'] = 'The provided API key is invalid.';
                    $response['charge'] = "0";
                    ...
                    $response['duration'] = "0";
                    return $response;
                } else if (isset($result[0]['condition']) && $result[0]['condition'] == "ROUTE_EXISTS") {
                    // indicating the API request was successful
                    
                    if (isset($result[0]['condition']) && $result[0]['condition'] == "ROUTE_EXISTS") {

                        $distance_text = $result[0]['distanceMeters'];
                        $distance_in_meter = $result[0]['distanceMeters'];
                        $distance = round(($distance_in_meter / 1000), 1);
                        ...
                        $response['duration'] = $time;
                        return $response;
                    } else if (isset($result[0]['condition']) && $result[0]['condition'] == "ROUTE_NOT_FOUND") {
                        $response['error'] = true;
                        $response['message'] = 'Data not found or invalid.Please check!';
                        $response['charge'] = "0";
                        $response['distance'] = "0";
...
function orders_count($status = "", $branch_id = "", $rider_id = "")
{
    $t = &get_instance();
    $t->db->select('count(DISTINCT o.id) total');
    if (!empty($status)) {
        $t->db->where('active_status', $status);
    }
    if (!empty($branch_id)) {
        $t->db->where('o.branch_id', $branch_id);
    }
    if (!empty($rider_id)) {
...
function get_price($type = "max")
{
    ...
    $t->db->select('IF( pv.special_price > 0, `pv`.`special_price`, pv.price ) as pr_price')
        ...
        ->join('`product_variants` pv', 'p.id = pv.product_id', 'LEFT')
        ->join('`product_attributes` pa', ' pa.product_id = p.id ', 'LEFT');
    $t->db->where(" `p`.`status` = '1' AND `pv`.`status` = 1 AND (`c`.`status` = '1' OR `c`.`status` = '0')");
...
function get_working_hour_html($restro_id = "", $is_time = true)
{
    ...
    if (isset($restro_id) && !empty($restro_id)) {
        $working_hours = fetch_details(["branch_id" => $restro_id], "branch_timings");
    }
    $days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    for ($i = 0; $i < count($days); $i++) {
        ...
        if (in_array($days[$i], $work)) {
            for ($j = 0; $j < count($working_hours); $j++) {
                if ($working_hours[$j]['day'] == $days[$i]) {
                    ...
                    $working_hours1 .= '<div id="' . $days[$i] . '" class="day">
                                            <div id="label" class="col-sm-3 col-form-label">' . $days[$i] . ': </div>
                                             ...
                                             to <input type="time" id="' . $days[$i] . 'ToH" name="' . $days[$i] . 'ToH" value="' . $closing_time . '"  class="hour from mr-2" ' . $is_disabled . ' >
                                                <input type="checkbox" name="not_working_days[]" value="' . $days[$i] . '" class="closed" ' . $is_open . ' >
            ...
            $working_hours1 .= '<div id="' . $days[$i] . '" class="day">
                                            ...
                                            <input type="checkbox" name="not_working_days[]" value="' . $days[$i] . '" class="closed"  >
                                            <span> Open</span>
                                    </div>';
        }
    }
 ...
 * 
 * @param int $points_polygon Number of vertices in the polygon
 * @param array $vertices_x Array of x-coordinates of the polygon vertices
 * @param array $vertices_y Array of y-coordinates of the polygon vertices
 * @param float $longitude_x X-coordinate of the point to test
 * @param float $latitude_y Y-coordinate of the point to test
...
function is_in_polygon($points_polygon, $vertices_x, $vertices_y, $longitude_x, $latitude_y) {
    ...
    if (count($vertices_x) !== $points_polygon || count($vertices_y) !== $points_polygon) {
        return false; // Arrays must match the number of points
    }
    ...
    for ($i = 0, $j = $points_polygon - 1; $i < $points_polygon; $j = $i++) {
        ...
        if ((($vertices_y[$i] > $latitude_y) !== ($vertices_y[$j] > $latitude_y)) &&
            ...
            ($latitude_y - $vertices_y[$i]) / ($vertices_y[$j] - $vertices_y[$i]) + 
            $vertices_x[$i])) {
            $inside = !$inside;
        }
    }
...
{
    $global_time = fetch_details(['id' => $id], 'branch', 'global_branch_time')[0];
    if ($global_time['global_branch_time'] == 1) {

        if (isset($id) && !empty($id)) {
            $t = &get_instance();
            $t->db->select('id');
            $t->db->where("day = DAYNAME(CURDATE())  and opening_time <= CURTIME() and closing_time >= CURTIME() and is_open=1 and branch_id = $id");
            $result = $t->db->from("branch_timings")->get()->result_array();
            if (!empty($result)) {
                return true;
            } else {
                return false;
...
function is_order_deliverable($address_id, $latitude_y, $longitude_x, $branch_id, $type = "address")
{
    if (!empty($branch_id) || $branch_id != "") {
        // get branch points
        $branch_data = fetch_details(['id' => $branch_id], "branch", "latitude,longitude");
        ...
        // here we are getting branch distance and points
        $data = fetch_details(['id' => $branch_id], "branch", "latitude,longitude,$where,city_id");
    
        if ($type == "city") {

            $branch = fetch_details(['id' => $address_id], 'cities', 'geolocation_type,radius,boundary_points,max_deliverable_distance');
        } else {
            $branch = fetch_details(['id' => $data[0]['city_id']], 'cities', 'geolocation_type,radius,boundary_points,max_deliverable_distance');

        }
        $city_distance = $branch[0]['max_deliverable_distance'];
        
        if (isset($branch) && !empty($branch) && isset($branch[0]['geolocation_type']) && $branch[0]['geolocation_type'] == "polygon") {

            $vertices_x = array_column(json_decode($branch[0]['boundary_points'], true), "lng");    // lng x-coordinates of the vertices of the polygon
            $vertices_y = array_column(json_decode($branch[0]['boundary_points'], true), "lat");    // lat y-coordinates of the vertices of the polygon
        ...
        } else if (isset($branch) && !empty($branch) && $branch[0]['geolocation_type'] == "circle") {
            // check for distance
            $distance = $data[0]['distance'];
            if ($distance <= $city_distance) {
...
function send_rider_notifications($role_id = "", $role = "rider", $title = "", $body = "", $type = "", $id = "")
{
    if ($role == "rider") {
        ...
        if (!empty($riders[0]['fcm_id'])) {
            $fcmMsg = array(
                'title' => $title,
                'body' => 'Hello Dear ' . $riders[2]['username'] . $body,
                'type' => $type,
                'content_available' => true
            );
            $fcm_ids[0][] = $riders[2]['fcm_id'];
            send_notification($fcmMsg, $fcm_ids, $fcmMsg, $fcmMsg['title'], $fcmMsg['body'], $fcmMsg['type']);
        }
        return true;
    } else {
...
function send_notifications($role_id = "", $role = "user", $title = "", $body = "", $type = "", $id = "")
{
    ...
    if ($role == 'admins') {
        ...
        $user_roles = fetch_details("", "user_permissions up", 'u.fcm_id,u.web_fcm_id,u.platform', "", "", "", "", "", "", 'users u', 'u.id=up.user_id');
        foreach ($user_roles as $user) {
            ...
            if (!empty($fcm_ids)) {
                ...
                send_notification($fcmMsg, $fcm_ids, $fcmMsg, $title, $body, $type);
            }
        }
        // foreach ($user_roles as $user) {
        //     $user_res = fetch_details(['id' => $user['user_id']], 'users', 'fcm_id,web_fcm_id,platform');
        //     if (isset($user_res[0]['fcm_id']) && !empty($user_res[0]['fcm_id'])) {

        //         $fcm_ids[0][] = $user_res[0]['fcm_id'];
        //     }
        //     if (isset($user_res[0]['web_fcm_id']) && !empty($user_res[0]['web_fcm_id'])) {
        ...
        $user_res = fetch_details(['id' => $role_id], 'users', 'username,fcm_id,web_fcm_id,email,platform');


        // if (!empty($user_res[0]['fcm_id']) || !empty($user_res[0]['web_fcm_id'])) {
        //     $fcmMsg = array(
        //         'title' => $title,
        ...
        if (!empty($user_res[0]['fcm_id'])) {
            ...
            send_notification($fcmMsg, $fcm_ids_test, $fcmMsg, $title, $body, $type);
        ...
        $riders = $t->db->select("u.serviceable_city,u.fcm_id,u.web_fcm_id,u.username")
            ->where("FIND_IN_SET('" . $id . "', u.serviceable_city) >", 0)
            ->get('users u')
            ->result_array();

        foreach ($riders as $key => $rider) {
            // if (!empty($rider['fcm_id']) || !empty($rider['web_fcm_id'])) {
            //     $fcmMsg = array(
            //         'title' => $title,
            //         'body' => 'Hello Dear' . $rider['username'] . $body,
            //         'type' => $type,
            //         'content_available' => true
            //     );
            //     if (isset($rider['fcm_id']) && !empty($rider['fcm_id'])) {

            //         $fcm_ids[0][] = $rider['fcm_id'];

            //     }
            //     if (isset($rider['web_fcm_id']) && !empty($rider['web_fcm_id'])) {

            //         $fcm_ids[0][] = $rider['web_fcm_id'];
            ...
            if (!empty($riders[0]['fcm_id'])) {
                ...
                $fcmMsg = array(
                    'title' => $title,
                    'body' => 'Hello Dear ' . $riders[0]['username'] . "\n" . $body,
                    'type' => (string)$type,
                    'type_id' => (string)$id,
                );
            }
...
function get_access_token()
{
    ...
    $privateKeyFile = FCPATH . 'uploads/' . $file_name;
    ...
    foreach ($chunk as $registrationID) {
        ...
        if ($platform == 'android') {
            $data["message"]["android"] = [
                "data" => [
                    "title" => (string)$customBodyFields['title'],
                    "body" => (string)$customBodyFields['body'],
                    "type" => (string)$customBodyFields['type'],
                    "type_id" => (isset($customBodyFields['type_id']) && !empty($customBodyFields['type_id'])) ? (string)$customBodyFields['type_id'] : '',
                    "image" => (isset($customBodyFields['image']) && !empty($customBodyFields['image'])) ? (string)$customBodyFields['image'] : '',
                ]
            ];
        } elseif ($platform == 'ios') {
            $data["message"]["apns"] = [
...
function send_notification($fcmMsg, $registrationIDs_chunks, $customBodyFields = [], $title = "test title", $message = "test message", $type = "test type", $platform_type = null)
{
    ...
    if (array_key_exists('', $registrationIDs_chunks)) {
        $registrationIDs_chunks['ios'] = $registrationIDs_chunks[''];
        unset($registrationIDs_chunks['']);
    }
    ...
    if ($platform_type) {
        ...
        $platforms_to_process = ['android', 'ios', 'web'];
    }
    ...
    foreach ($platforms_to_process as $platform) {
        if (isset($registrationIDs_chunks[$platform])) {
            foreach ($registrationIDs_chunks[$platform] as $chunk) {
                sendNotificationToPlatform($platform, $chunk, $url, $headers, $customBodyFields, $title, $message);
            }
        }
    }
...
function update_rider($rider_id, $order_id, $status = "accepted")
{
    ...
    $user_res = fetch_details(['id' => $rider_id], 'users', 'fcm_id,web_fcm_id,username,platform');
    $fcm_ids = array();
    $msg = "";
    if (isset($user_res[0]) && !empty($user_res[0])) {
        ...
        if (isset($current_rider[0]['rider_id']) && $current_rider[0]['rider_id'] == $rider_id) {
            ...
            if (isset($current_rider[0]['rider_id']) && !empty($current_rider[0]['rider_id']) && $current_rider[0]['rider_id'] != $rider_id) {
                $delivery_error = false;
                if (update_details(['rider_id' => $rider_id], ['id' => $order_id], 'orders')) {

                    $msg = "Order re-assigned to other Rider.";
                }
            } else {
               
                $custom_notification = fetch_details(['type' => 'rider_assign_order'], 'custom_notifications', '*');

                $hashtag_rider_name = '< rider_name >';
                $hashtag_order_id = '< order_id >';
                $hashtag_application_name = '< application_name >';
                $string = json_encode($custom_notification[0]['message'], JSON_UNESCAPED_UNICODE);
                $hashtag = html_entity_decode($string);
                $data = str_replace(array($hashtag_rider_name, $hashtag_order_id, $hashtag_application_name), array($user_res[0]['username'], $order_id, $app_name), $hashtag);
                $message = output_escaping(trim($data, '"'));
                $fcmMsg = array(
                    'title' => (!empty($custom_notification)) ? $custom_notification[0]['title'] : "You have new order to deliver",
                    'body' => (!empty($custom_notification)) ? $message : 'Hello Dear ' . $user_res[0]['username'] . ' you have new order to be deliver order ID #' . $order_id . '. Order details you can take from app order details. Please take note of it! Thank you. Regards ' . $app_name . '',
                    'type' => "order"
                );
                $msg = 'Rider Updated.';
...
function calculate_delivery_time($distance, $time_to_travel_city, $cooking_time = 0)
{
    if ($distance > 0) {
...
function remove_dine_in_cart($table_id)
{
...
function save_branch($data = NULL)
{
    
    if (isset($data) && !empty($data)) {
        $_SESSION['branch_id'] = $data;
...
if (!function_exists('is_url_active')) {
    function is_url_active(...$segments)
    {
        $CI = &get_instance(); // Get the CodeIgniter instance
        $CI->load->helper('url'); // Ensure the URL helper is loaded
        $current_url = current_url(); // Get the current URL
        // Check if any of the passed segments exist in the current URL
        foreach ($segments as $segment) {
            if (strpos($current_url, $segment) !== false) {
                return true; // Return true if any segment matches
            }
        }
        return false; // Return false if no segments match
    }
}
...
if (!function_exists('generate_breadcrumbs')) {
    function generate_breadcrumbs()
    {
        ...
        foreach ($segments as $key => $segment) {
            ...
            $name = ucwords(preg_replace('/[_-]/', ' ', $segment));

            // if ($segment === 'edit_orders') {
            //     $name = 'Edit Orders';
            // }
    ...
    function display_breadcrumbs()
    {
        ...
        foreach ($breadcrumbs as $index => $crumb) {
            if ($index == $total - 1) {
                ...
                $output .= '<li class="breadcrumb-item active">' . $crumb['name'] . '</li>';
            } else {
                // Linked items
                $output .= '<li class="breadcrumb-item"><a class="text text-info" href="' . $crumb['url'] . '">' . $crumb['name'] . '</a></li>';
            }
        }
