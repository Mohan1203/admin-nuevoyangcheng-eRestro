...
class Api extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model(['category_model', 'order_model', 'rating_model', 'cart_model', 'address_model', 'transaction_model', 'notification_model', 'Rider_model', 'Order_model', 'Area_model']);
    ...
    */
    ...
    public function register_rider()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            $this->response['error'] = false;
            $this->response['message'] = 'Rider Added Successfully, Please wait for approval from admin!';
            print_r(json_encode($this->response));
        }
    }
    ...
    public function update_user()
    {
        /*
            ...
            old:12345 {only required when update user}
            new:345234 {only required when update user}
            profile: image {optional}
            accept_orders:1 or 0  {optional}{Default:1} // this is for online status maintain for rider , if rider disable this staus he willl not be able to receive any order, however he can still login to the account.
        */
            // password: Test@123iuy {orly required when register rider}
            // serviceable_city: 1,2,3 {only required when register rider , dont allow to update city oonce rider is registerd}
        ...
        $this->form_validation->set_rules('user_id', 'User Id', 'xss_clean|numeric|trim');
        $this->form_validation->set_rules('username', 'Username', 'xss_clean|trim');
        $this->form_validation->set_rules('accept_orders', 'Accept Orders', 'xss_clean|trim|numeric');

        if (!empty($_POST['old']) || !empty($_POST['new'])) {
            $this->form_validation->set_rules('old', $this->lang->line('change_password_validation_old_password_label'), '');
            $this->form_validation->set_rules('new', $this->lang->line('change_password_validation_new_password_label'), 'min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']');
        }
        ...
        if (!$this->form_validation->run()) {
            ...
            if (isset($_POST['accept_orders']) && !empty($_POST['accept_orders'])) {
                if (defined('ALLOW_MODIFICATION') && ALLOW_MODIFICATION == 0) {
                    ...
                    $order_details = fetch_orders(false, false, $multiple_status, $_POST['user_id']);
                    if (!empty($order_details['order_data'])) {
                        $this->response['error'] = true;
                        $this->response['message'] = 'You have some order assigned, If you still want to deactive the account then you can contact to the Administrator!';
                        echo json_encode($this->response);
                        return;
                    }else{
                        $set['accept_orders'] = $this->input->post('accept_orders', true);
                    }
        ...
        if (!$this->form_validation->run()) {
            ...
            if (isset($_POST['mobile']) && is_exist(['mobile' => $_POST['mobile']], 'users')) {
                ...
                if (!$this->ion_auth->in_group('rider', $user_id[0]['id'])) {
                    $this->response['error'] = true;
                    $this->response['message'] = 'Mobile number could not be found!';
                    print_r(json_encode($this->response));
                    return;
                } else {
                    $this->response['error'] = false;
    ...
    public function get_settings()
    {
        ...
        $settings_res['authentication_mode'] = $authentication_mode['authentication_method'] == 'sms' ? 1 : 0;
    ...
    public function update_order_request()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            $order_id = $this->input->post('order_id', true);
            $rider_id = $this->input->post('rider_id', true);
            $can_accept_oredr = fetch_details(['id' => $rider_id], 'users', 'accept_orders');
            if ($can_accept_oredr[0]['accept_orders'] == 0) {
                $this->response['error'] = true;
                $this->response['message'] = "You are not allowed to accept order.";
                print_r(json_encode($this->response));
                return false;
            }
