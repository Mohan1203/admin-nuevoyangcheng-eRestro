...
class Orders extends CI_Controller
{
    ...
    public function view_pending_orders()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_rider()) {
            ...
            $rejected_rider = $this->ion_auth->get_user_id();
            $city_id = fetch_details(['id' => $deliveryBoyId, 'active' => '1'], "users", 'serviceable_city');

           
            $test = $this->Order_model->get_orders_list(null, true, $city_id[0]['serviceable_city'], '', '', '', 'DESC', $rejected_rider);
    ...
    public function edit_orders()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_rider()) {
            ...
            $this->data['branch_data'] = fetch_details(['id' => $res[0]['branch_id']], 'branch', '*');
    ...
    /* To update the status of particular order  */
    public function update_order_status()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $where_id = "id = " . $order_id;
            
            if (isset($order_id) && isset($field) && isset($val)) {
                if ($field == 'status') {

                    $order_details = fetch_details($where_id, 'order_items', 'user_id,order_id');
                    $current_orders_status = fetch_details(['id' => $order_details[0]['order_id']], 'orders', 'active_status,user_id');
                    $user_id = $current_orders_status[0]['user_id'];
                    $current_orders_status = $current_orders_status[0]['active_status'];
                    $main_order_id = "id = " . $order_details[0]['order_id'] . " and (active_status != 'cancelled' ) ";
                    if ($priority_status[$val] > $priority_status[$current_orders_status]) {
                        $set = [
                            $field => $val 
                        ];
                        /* Update Active Status of Order Table */
                        if ($this->Order_model->update_order($set, $main_order_id, true)) {
                            if ($this->Order_model->update_order(['active_status' => $val], $main_order_id)) {
                                $error = false;
                            }
                        }
                        ...
                        if ($error == false) {
                            ...
                            $body = (!empty($custom_notification)) ? $message :  ' Order status updated to ' . $val . ' for your order ID #' . $main_order_id . ' please take note of it! Thank you for ordering with us.';
                            send_notifications($user_id, "user", $title, $body, "order", $main_order_id);

                            /* Process refund when order cancel */
                            process_refund($main_order_id, $val, 'orders');
                            if (trim($val) == 'cancelled') {
                                $data = fetch_details(['order_id' => $main_order_id], 'order_items', 'product_variant_id,quantity');
                            ...
                            $response = process_referral_bonus($user_id, $main_order_id, $val);
    ...
    public function get_otp_setting()
    {

        // $settings = get_settings('system_settings', true);
        $is_rider_otp_setting_on = fetch_orders($_POST['order_id']);
        
        $this->response['error'] = false;
        $this->response['message'] = 'OTP settings retrived!';
        $this->response['result'] = isset($is_rider_otp_setting_on[0]) && !empty($is_rider_otp_setting_on[0]) ? $is_rider_otp_setting_on[0] : get_settings('system_settings', true);
        $this->response['csrfName'] = $this->security->get_csrf_token_name();
        $this->response['csrfHash'] = $this->security->get_csrf_hash();
        print_r(json_encode($this->response));
    }
    ...
    public function update_order_request()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_rider()) {
            ...
            if($can_accept_oredr[0]['accept_orders'] == 0){
                $this->response['error'] = true;
                $this->response['message'] = "You have deactivated accept order.";
                $this->response['data'] = array();
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                print_r(json_encode($this->response));
