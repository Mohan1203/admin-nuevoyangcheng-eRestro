...
class Promo_code extends CI_Controller
{
    ...
    public function add_promo_code()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->form_validation->set_rules('image', 'Main Image', 'trim|required|xss_clean', array('required' => 'Main image is required'));
            ...
            $this->form_validation->set_rules('image', 'Promocode Image ', 'trim|required|xss_clean');
            ...
            if (!$this->form_validation->run()) {
                ...
                if (isset($_POST['edit_promo_code'])) {
                    ...
                    if (is_exist(['promo_code' => $_POST['promo_code'], 'branch_id' => $_SESSION['branch_id']], 'promo_codes')) {
                        $response["error"] = true;
                        $response['csrfName'] = $this->security->get_csrf_token_name();
                        $response['csrfHash'] = $this->security->get_csrf_hash();
