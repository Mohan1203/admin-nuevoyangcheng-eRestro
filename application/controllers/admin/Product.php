...
class Product extends CI_Controller
{
    ...
    public function product_order()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $this->data['categories'] = $this->category_model->get_categories();
            $products = $this->db->select('*')->where('branch_id', $_SESSION['branch_id'])->order_by('row_order')->get('products')->result_array();
    ...
    public function search_category_wise_products()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_SESSION['branch_id'])) {
                $this->db->where('p.branch_id', $_SESSION['branch_id']);
            }
    ...
    public function add_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if(isset($_POST['available_time']) && $_POST['available_time'] == 'on'){
                $this->form_validation->set_rules('product_start_time', 'Product Start Time', 'trim|required|xss_clean');
                $this->form_validation->set_rules('product_end_time', 'Product End Time', 'trim|required|xss_clean');
            }

            $minimum_order_qty = $_POST['minimum_order_quantity'];
            $total_allowed_quantity = $_POST['total_allowed_quantity'];

            if (!empty($minimum_order_qty) && !empty($total_allowed_quantity)) {

                if ($minimum_order_qty > $total_allowed_quantity) {
                    $this->response['error'] = true;
                    $this->response['csrfName'] = $this->security->get_csrf_token_name();
                    $this->response['csrfHash'] = $this->security->get_csrf_hash();
                    $this->response['message'] = "Minimum Order Quantity can't be grater then Total Allowed Quantity";
            ...
            if (isset($_POST['product_type']) && $_POST['product_type'] == 'simple_product') {
                $this->form_validation->set_rules('simple_price', 'Price', 'trim|required|numeric|greater_than_equal_to[' . $this->input->post('simple_special_price') . ']|xss_clean|greater_than[0]');
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->response['location'] = base_url('admin/product/');
                print_r(json_encode($this->response));
            }
        } else {
            redirect('admin/login', 'refresh');
    ...
    public function get_product_data()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            $indicator =  (isset($_GET['indicator']) && $_GET['indicator'] != "") ? $this->input->get('indicator', true) : NULL;
            $product_type =  (isset($_GET['type']) && $_GET['type'] != "") ? $this->input->get('type', true) : NULL;

            if (isset($_GET['flag']) && !empty($_GET['flag'])) {
                return $this->product_model->get_product_details($_GET['flag'], $status, $indicator,$product_type);
            }
            return $this->product_model->get_product_details(null, $status, $indicator,$product_type);
        } else {
            redirect('admin/login', 'refresh');
        }
    }
    ...
    public function get_slider_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            return $this->product_model->get_product_details(null, $status = 1);
        } else {
            redirect('admin/login', 'refresh');
        }
    }
    ...
    public function view_product()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                ...
                if($res['product'][0]['branch_id'] == $_SESSION['branch_id']){
                ...
                if ($type == 'upload') {
                    while (($row = fgetcsv($handle, 10000, ",")) != FALSE) //get row values
                    {
                        if ($temp != 0) {
                            ...
                            if (!empty($row[0]) && !empty($row[19])) {
                                $verify_branch_category = fetch_details(['id' => $row[0], 'branch_id' => $row[19]], 'categories', '*');
                                if (empty($verify_branch_category)) {
                                    $response["error"]   = true;
                                    $response["message"] = "Branch id is invalid for the category at row." . $temp;
                                    $response['csrfName'] = $this->security->get_csrf_token_name();
                                    $response['csrfHash'] = $this->security->get_csrf_hash();
                                    $response["data"] = array();
                                    echo json_encode($response);
