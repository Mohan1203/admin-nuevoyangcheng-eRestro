...
class Riders extends CI_Controller
{
    ...
    public function view_rider_requests()
    {
        ...
        public function view_riders_cash_collection()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (delete_details(['id' => $_GET['id']], 'users') == TRUE) {
                if (delete_details(['user_id' => $_GET['id']], 'users_groups') == TRUE) {
                    ...
                    $this->response['message'] = 'User removed from Rider successfully';
    ...
    public function add_rider()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_POST['commission_method']) && !empty($_POST['commission_method']) && $_POST['commission_method'] == "percentage_on_delivery_charges") {
                ...
                $this->form_validation->set_rules('max_commission', 'Max Commission', 'trim|xss_clean|required');
            }
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->response['messages'] = array(
                    ...
                    'max_commission' => form_error('max_commission'),
                ...
                $_POST['max_commission'] = isset($max_commission) ? $max_commission : "";
                ...
                if (isset($_POST['edit_rider']) || isset($_POST['edit_rider_request'])) {
                   ...
                   if(isset($_POST['edit_rider_request'])) {
                    ...
                    if(isset($_POST['edit_rider_request'])) {
                        $_POST['branch_id'] = $this->input->post("branch", true);
                    ...
                    if (validatePassword($password)) {
                        $additional_data = [
                            ...
                            'max_commission' => isset($max_commission) ? $max_commission : "0",
                        ...
                        update_details(['active' => $_POST['active']], [$identity_column => $identity], 'users');
                    } else {
                        $response["error"]   = true;
    ...
    public function rider_registration_request(){
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            if (!has_permissions('read', 'rider_requests')) {
                $this->session->set_flashdata('authorize_flag', PERMISSION_ERROR_MSG);
                redirect('admin/home', 'refresh');
            }
