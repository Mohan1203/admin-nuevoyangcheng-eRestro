    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                $slider_data = fetch_details(['id' => $_GET['edit_id']], 'sliders', '*');
                if ($slider_data[0]['branch_id'] !== $_SESSION['branch_id']) {
                    redirect('admin/slider/manage-slider/', 'refresh');
                }
            }
            ...
            $this->data['categories'] = fetch_details(['status' => 1, 'branch_id' => $_SESSION['branch_id']], 'categories', '*');
            }else{
                $this->data['categories'] = [];
                $this->data['permissions_message'] = PERMISSION_ERROR_MSG;
            }
            if (isset($_GET['edit_id']) && !empty($_GET['edit_id'])) {
                $this->data['fetched_data'] = fetch_details(['id' => $_GET['edit_id']], 'sliders');
            }
    ...
    public function add_slider()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $this->response['messages'] = array(
                'slider_type' => form_error('slider_type'),
            ...
            $this->response['csrfName'] = $this->security->get_csrf_token_name();
            $this->response['csrfHash'] = $this->security->get_csrf_hash();
            $this->response['location'] = base_url('admin/slider/manage_slider');
