...
class Customer extends CI_Controller
{
    ...
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            return $this->Customer_model->get_top_customers(5);
        } else {
            redirect('admin/login', 'refresh');
        }
    }

    public function view_customer_wallet_transaction()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            return $this->Customer_model->get_customer_list($wallet_transaction = true);
        } else {
            redirect('admin/login', 'refresh');
        }
    }
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                if ($_POST['type'] == 'debit' || $_POST['type'] == 'credit') {
                    $response = update_wallet_balance($_POST['type'], $_POST['user_id'], $_POST['amount'], $_POST['message']);
