    ...
    public function manage_ticket_types()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (isset($_GET['edit_id'])) {
                $this->data['fetched_data'] = fetch_details(['id' => $_GET['edit_id']], 'ticket_types');
            }
            $this->load->view('admin/template', $this->data);
        } else {
            redirect('admin/login', 'refresh');
    ...
    public function add_ticket_type()
    {
        ...
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) {
            ...
            if (!$this->form_validation->run()) {
                ...
                $this->response['csrfName'] = $this->security->get_csrf_token_name();
                $this->response['csrfHash'] = $this->security->get_csrf_hash();
                $message = (isset($_POST['edit_ticket_type'])) ? 'Ticket Updated Successfully' : 'Ticket Added Successfully';
    ...
    public function get_ticket_messages()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $this->response['message'] = array(
                    'ticket_id' => form_error('ticket_id'),                    
                );
            print_r(json_encode($this->response));
        } else {
            ...
            if (!$this->form_validation->run()) {
                ...
                if (!$this->ticket_model->add_ticket($data)) {
                    ...
                    if (!empty($result)) {
                        ...
                        $user_res = fetch_details(['id' => $ticket_res[0]['user_id']], "users", 'fcm_id,web_fcm_id', '',  '', '', '');
