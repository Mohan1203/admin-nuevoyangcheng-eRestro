...
class Customer_model extends CI_Model
{
    ...
    public function get_customer_list($wallet_transaction = false)
    {
        ...
        $cat_search_res = $search_res->group_by('u.id')->order_by($sort, $order)->limit($limit, $offset)->get('users u')->result_array();
        ...
        foreach ($cat_search_res as $row) {
            ...
            $row['profile'] = isset($row['image']) && !empty($row['image']) ? base_url() . USER_IMG_PATH . $row['image'] : base_url() . NO_PROFILE_IMAGE;
            $tempRow['profile'] = '<div class="mx-auto product-image image-box-100"><a href=' . $row['profile'] . ' data-toggle="lightbox" data-gallery="gallery">
            <img src=' . $row['profile'] . ' class="img-fluid rounded"></a></div>';
    ...
    {
        // Select user details and the count of their orders
        $this->db->select('users.*, COUNT(orders.id) as total_orders');
        $this->db->from('users');
        $this->db->join('orders', 'users.id = orders.user_id', 'left');
        $this->db->where('orders.branch_id', $_SESSION['branch_id']);  // Add this line
        ...
        if ($query->num_rows() > 0) {
            $top_customers =  $query->result_array(); // Return the result as an array of objects
            ...
            foreach ($top_customers as $row) {
                ...
                $tempRow['name'] = $row['username'];
                $row['profile'] = isset($row['image']) && !empty($row['image']) ? base_url() . USER_IMG_PATH . $row['image'] : base_url() . NO_PROFILE_IMAGE;
                $tempRow['profile'] = '<div class="mx-auto product-image image-box-100">
    <a href="' . $row['profile'] . '" data-toggle="lightbox" data-gallery="gallery">
        <img src="' . $row['profile'] . '" class="img-fluid rounded-circle profile-image">
    </a>
</div>';
                ...
                $tempRow['total_orders'] = "<p>&nbsp;</p>" . $row['total_orders'];
                $tempRow['balance'] = number_format($row['balance'], 2);
                $tempRow['status'] = ($row['active'] == '1') ? '<a class="badge badge-success text-white" >Active</a>' : '<a class="badge badge-danger text-white" >Inactive</a>';
                $tempRow['actions'] = $operate;
            ...
            return array(); // Return an empty array if no customers found
        }
    }

    public function update_balance($amount, $rider_id, $action)
    {
        /**
         * @param
         * action = deduct / add
