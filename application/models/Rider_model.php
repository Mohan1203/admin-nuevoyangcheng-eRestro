...
class Rider_model extends CI_Model
{
    ...
    function update_rider($data, $image)
    {
        ...
        $rider_data = [
            ...
            'serviceable_city' => isset($data['serviceable_city']) && !empty($data['serviceable_city']) ? implode("," , $data['serviceable_city']) : "",
            'active' => (isset($data['active']) && $data['active'] != "") ? $data['active'] : 1,
            'rider_cancel_order' => (isset($data['rider_cancel_order']) && $data['rider_cancel_order'] != "") ? $data['rider_cancel_order'] : 0,
            'image' => (isset($image) && $image != "") ? $image : 1,
        ];
        if(isset($data['edit_rider'])) {
            $this->db->set($rider_data)->where('id', $data['edit_rider'])->update('users');
            
        }else{
            $rider_data['branch_id'] = $data['branch_id'];
            $this->db->set($rider_data)->where('id', $data['edit_rider_request'])->update('users');

        }
        return $this->db->last_query();
    }

    function get_riders_list($cash_collection = false, $rider_requests = false)
    {
        ...
        if (isset($where) && !empty($where)) {
            if ($rider_requests == true) {
                $where['u.active'] = 2;
            } else {

                $where['u.branch_id'] = $branch_id;
            }
            if($cash_collection == true){
            $where['u.active'] = 1;
        ...
        foreach ($offer_search_res as $row) {
            ...
            $row = output_escaping($row);
                ...
                if($rider_requests == true){
                    $operate = "<a href=" . base_url('admin/riders?edit_rider_request=' . $row['id'] . '') . " data-id=" . $row['id'] . " class='btn btn-success btn-xs mr-1 mb-1' title='Edit' ><i class='fa fa-pen'></i></a>";
                    $operate .= '<a  href="javascript:void(0)" class="btn btn-danger btn-xs mr-1 mb-1" title="Delete" id="delete-riders"  data-id="' . $row['id'] . '" ><i class="fa fa-trash"></i></a>';
                ...
                $tempRow['branch'] = isset($branch_name[0]['branch_name']) && !empty($branch_name[0]['branch_name']) ? stripslashes($branch_name[0]['branch_name']) : "";
    ...
    public function get_riders($id, $search, $offset, $limit, $sort, $order,$branch_id)
    {
        $multipleWhere = '';
        $where['ug.group_id'] =  3;
        if (!empty($search)) {
    ...
    public function get_rider_cash_collection($limit = "", $offset = '', $sort = 'transactions.id', $order = 'DESC', $search = NULL, $filter = "")
    {
        ...
        if (isset($search) and $search != '') {
            $multipleWhere = ['`transactions.id`' => $search, '`transactions.amount`' => $search, '`transactions.date_created`' => $search, 'users.username' => $search, 'users.mobile' => $search, 'users.email' => $search, 'transactions.type' => $search, 'transactions.status' => $search];
        }
        ...
        $bulkData['message'] = (empty($txn_search_res)) ? 'Cash collection does not exist' : 'Cash collection retrieved successfully';
    ...
    function get_cash_collection_list($user_id = '')
    {
        ...
        if (!empty($user_id)) {
            $user_where = ['users.id' => $user_id, 'users.branch_id' => $_SESSION['branch_id']];
            
        }
        // $user_where = ['users.branch_id' => $_SESSION['branch_id']];
