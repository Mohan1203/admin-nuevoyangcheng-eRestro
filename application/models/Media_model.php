...
class Media_model extends CI_Model
{
    ...
    {
        if (($this->ion_auth->logged_in() && $this->ion_auth->is_admin()) || ($this->ion_auth->logged_in() && $this->ion_auth->is_partner() && ($this->ion_auth->partner_status() == 1 || $this->ion_auth->partner_status() == 0))) {
            ...
            foreach ($city_search_res as $row) {
                ...
                $tempRow['image'] = '<div class="image-upload-div text-center"><span class="path d-none">' . base_url() . $row['sub_directory'] . $row['name'] . '</span><span class="relative-path d-none">' . $row['sub_directory'] . $row['name'] . '</span><a href="' . $row['image'] . '" data-toggle="lightbox" data-gallery="gallery"><img src="' . $row['image'] . '" width="200" height="150"></a></div>';
    ...
    public function get_media($limit = "", $offset = '', $sort = 'id', $order = 'DESC', $search = NULL, $type = "", $partner_id = NULL)
    {
    ...
    {
        // Ensure that IDs are passed as an array
        $this->db->where_in('id', $ids);
        $this->db->delete('media'); // Replace 'media' with your actual table name
    }
}
