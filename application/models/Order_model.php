...
class Order_model extends CI_Model
{
                    ...
                    if ($current_status == 'delivered') {
                        ...
                        if (!empty($order)) {
                            ...
                            if ($rider_id > 0) {
                                ...
                                $rider = fetch_details("id = $rider_id", 'users', 'commission_method,commission,max_commission');
                                ...
                                if (!empty($rider)) {
                                    ...
                                    if (isset($rider[0]['commission_method']) && $rider[0]['commission_method'] == 'percentage_on_delivery_charges') {
                                        ...
                                        $max_commission = (isset($rider[0]['max_commission']) && !empty($rider[0]['max_commission']) && $rider[0]['max_commission'] > 0) ? $rider[0]['max_commission'] : 0;
    ...
    public function place_order($data)
    {
        ...
        $total = 0;
        $product_variant = [];
        for ($s = 0; $s < count($product_variant_id); $s++) {
            $product_variants = $this->db->select('pv.*,tax.percentage as tax_percentage,tax.title as tax_name,p.name as product_name,p.is_prices_inclusive_tax')
                ->join('products p ', 'pv.product_id=p.id', 'left')
                ->join('categories c', 'p.category_id = c.id', 'left')
                ->join('`taxes` tax', 'tax.id = p.tax', 'LEFT')
                ->where('pv.id', $product_variant_id[$s])->order_by('FIELD(pv.id,' . $data['product_variant_id'] . ')')->get('product_variants pv')->result_array();
            foreach ($product_variants as $variant) {
                $variant['cart_id'] = $cart_id[$s];
        ...
        for ($i = 0; $i < count($product_variant); $i++) {
            ...
            $add_ons = get_cart_add_ons($product_variant[$i]['id'], $product_variant[$i]['product_id'], $data['user_id'], $product_variant[$i]['cart_id']);
        ...
        if (isset($data['promo_code']) && !empty($data['promo_code'])) {
            ...
            $promo_code = validate_promo_code($data['promo_code'], $data['user_id'], $data['total'], $branch_id);
        ...
        $final_total = $total + $delivery_charge + $delivery_tip + $tax_amount - $promo_code_discount;
        ...
        $order_data = [
            ...
            'is_rider_otp_setting_on' => isset($is_rider_otp_setting_on['is_rider_otp_setting_on']) && $is_rider_otp_setting_on['is_rider_otp_setting_on'] == "1" ? $is_rider_otp_setting_on['is_rider_otp_setting_on'] : "0",
            'branch_id' => $branch_id,
            'mobile' => $data['mobile'],
    ...
    public function get_orders_list(
        ...
        if(isset($rejected_rider)){

            // using rejected_rider id here because while loading the pending order it send current riders id so using it to get if the rider's accept order is enable or not.
            $accept_order = fetch_details(['id' => $rejected_rider], 'users', 'accept_orders');
        ...
        //             $temp .= "<b>ID :</b>" . $item['id'] . "<b> Product Variant Id :</b> " . $item['product_variant_id'] . "<b> Variants :</b> " . $variants . "<b> Name : </b>" . stripslashes($item['name']) . " <b>Price : </b>" . $item['price'] . " <b>QTY : </b>" . $item['quantity'] . " <b>Subtotal : </b>" . $item['quantity'] * $item['price'] . "<br>------<br>";
        ...
        //         }
        //         if (trim($row['active_status']) == 'draft') {
        ...
        //             if($latest_orders == 1) {
        //                 $operate = '<div class="btn-group" role="group" aria-label="Order Actions">
        //         <a href="' . base_url('admin/orders/edit_orders') . '?edit_id=' . $row['id'] . '" 
        ...
        foreach ($user_details as $row) {
            if (!empty($row['items'])) {
                ...
                // Limit display to first 2 items only
                $display_items_count = min(2, count($items));
                $total_items_count = count($items);
                
                // Process first 2 items for display
                for ($i = 0; $i < $display_items_count; $i++) {
                    $item = $items[$i];
                    $product_variants = get_variants_values_by_id($item['product_variant_id']);
                    $variants = isset($product_variants[0]['variant_values']) && !empty($product_variants[0]['variant_values']) ? str_replace(',', ' | ', $product_variants[0]['variant_values']) : '-';
                    $temp .= "<b>ID :</b>" . $item['id'] . "<b> Product Variant Id :</b> " . $item['product_variant_id'] . "<b> Variants :</b> " . $variants . "<b> Name : </b>" . stripslashes($item['name']) . " <b>Price : </b>" . $item['price'] . " <b>QTY : </b>" . $item['quantity'] . " <b>Subtotal : </b>" . $item['quantity'] * $item['price'] . "<br>------<br>";
                ...
                // Add "Show More" button if there are more than 2 items
                if ($total_items_count > 2) {
                    // Calculate remaining items
                    $remaining_items = $total_items_count - $display_items_count;
                    
                    // Calculate total amount and quantity for all items
                    foreach ($items as $item) {
                        if (!isset($total_amt)) {
                            $total_amt += $item['sub_total'];
                            $total_qty += $item['quantity'];
                    ...
                    // $temp .= "<div class='text-center'><a href='javascript:void(0)' class='btn btn-sm btn-primary show-more-items' data-order-id='{$order_id}'>Show {$remaining_items} More Items</a></div>";
                    // Add "Show More" button with direct link
                    $view_url = '';
                    if ($this->ion_auth->is_rider()) {
                        $view_url = base_url('rider/orders/edit_orders') . '?edit_id=' . $order_id;
                    } else if ($this->ion_auth->is_partner()) {
                        $view_url = base_url('partner/orders/edit_orders') . '?edit_id=' . $order_id;
                    } else if ($this->ion_auth->is_admin()) {
                        $view_url = base_url('admin/orders/edit_orders') . '?edit_id=' . $order_id;
                    }

                    $temp .= "<div class='text-center'><a href='{$view_url}' class='btn btn-sm btn-primary'>Show {$remaining_items} More Items</a></div>";
                }
    ...
    public function pos_place_order($data)
    {
        ...
        foreach ($product_variant_id as $variant_id) {
            // Run the query for each variant_id
            $variant = $this->db->select('pv.*,tax.percentage as tax_percentage,tax.title as tax_name,p.branch_id,p.name as product_name,p.is_prices_inclusive_tax')
            ->join('products p ', 'pv.product_id=p.id', 'left')
            ->join('categories c', 'p.category_id = c.id', 'left')
            ->join('`taxes` tax', 'tax.id = p.tax', 'LEFT')
            ->where('pv.id', $variant_id)
            ->get('product_variants pv')
            ->row_array(); // Fetch a single row for each iteration

            if (!empty($variant)) {
                $product_variant[] = $variant; // Add the variant data to the result array
            }
        }
        if (!empty($product_variant)) {
            ...
            $order_data['notes'] = $data['order_note'];
            $order_data['is_self_pick_up'] = isset($data['is_self_pick_up']) ? $data['is_self_pick_up'] : 2;
            $this->db->insert('orders', $order_data);
            $last_order_id = $this->db->insert_id();
          
            for ($i = 0; $i < count($product_variant); $i++) {
                // Filter add-ons specific to the current product_variant
                $filtered_add_ons = [];
                foreach ($add_ons_details as $add_on) {
                    if ($add_on['product_variant_id'] == $product_variant[$i]['id']) {
                        $filtered_add_ons[] = $add_on;
                    }
                }
               
                $json_encoded = json_encode($filtered_add_ons);
                $product_variant_data[$i] = [
                    'user_id' => $data['user_id'],
                    'order_id' => $last_order_id,
                    'tax_percent' => $tax,
                    ...
                    'sub_total' => $subtotal[$i],
                    'add_ons' => (!empty($json_encoded)) ?  $json_encoded  : '[]'
                ];
